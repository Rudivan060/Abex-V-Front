        ...
        <v-dialog v-model="ativo" max-width="500">
          <v-card height="550" width="500" theme="dark">
            ...
            <v-card-text>
              <v-row>
                <v-col>
                  <v-text-field variant="outlined" label="Código" placeholder="Código" v-model="user.zipCode">
                  </v-text-field>
                </v-col>
  
                <v-col>
                  <v-text-field variant="outlined" label="Tipo" placeholder="Tipo" v-model="user.state">
                  </v-text-field>
                </v-col>
              </v-row>
              ...
              <v-row>
                <v-col>
                  <v-text-field variant="outlined" label="Valor" placeholder="Valor" v-model="user.city">
                  </v-text-field>
                </v-col>
  
                <v-col>
                  <v-text-field variant="outlined" label="Quantidade" placeholder="Quantidade" v-model="user.street">
                  </v-text-field>
                </v-col>
              </v-row>
              ...
              <v-row>
                <v-col>
                  <v-text-field variant="outlined" label="Valor" placeholder="Valor" v-model="user.district">
                  </v-text-field>
                </v-col>
  
                <v-col>
                  <v-text-field variant="outlined" label="Quantidade" placeholder="Quantidade" v-model="user.numberForget">
                  </v-text-field>
                </v-col>
              </v-row>
              ...
              <v-row>
                <v-col>
                  <v-text-field variant="outlined" label="Quantidade" placeholder="Quantidade" v-model="user.idUser">
                  </v-text-field>
                </v-col>
              </v-row>
            </v-card-text>
      ...
      <v-layout>
        <v-navigation-drawer expand-on-hover rail theme="dark">
          ...
          <v-list density="compact" nav>
            ...
            <v-list-item href="http://localhost:3000/cruduser" prepend-icon="mdi-ticket-percent-outline"
    ...
    export default {
      ...
      computed: {
        tituloDialog: function () {
          return this.user.id ? "Editar" : "Criar";
        },
      },
      ...
      methods: {
        ...
        async persist() {
          if (this.user.id) {
            const response = await this.$api.patch(`/user/persist/${this.user.id}`, this.user);
          } else {
        ...
        async deleteItem(item) {
          if (confirm(`Deseja deletar?`)) {
            const response = await this.$api.delete(`/user/delete/${item.id}`);
          }
